...
/* 
    Show realname and email filters outside the "Show more" section by overriding their _advanced flag.
    @author Daniel Cárdenas Ortiz <daniel.cardenas.ortiz@correounivalle.edu.co>
*/
if (!class_exists('tool_browse_users_classic_filtering')) {
    class tool_browse_users_classic_filter_text_realname extends user_filter_text {
        /**
         * Make only the "User full name" text box wider.
         *
        ...
        public function setupForm(&$mform) {
            ...
            $objs['text'] = $mform->createElement('text', $this->_name, null, ['size' => 42]);
    ...
    class tool_browse_users_classic_filtering extends user_filtering {
        public function get_field($fieldname, $advanced) {
            // Force realname and email to be non-advanced (visible by default).
            if ($fieldname === 'realname' || $fieldname === 'email') {
                $advanced = false;
            }

            // Make only the realname filter input wider.
            if ($fieldname === 'realname') {
                return new tool_browse_users_classic_filter_text_realname(
                    'realname',
                    get_string('fullnameuser'),
                    $advanced,
                    $GLOBALS['DB']->sql_fullname()
                );
            }
...
$ufiltering = new tool_browse_users_classic_filtering();
echo $OUTPUT->header();
...
if (!$users) {
    ...
    // Disable query countries
    //$countries = get_string_manager()->get_list_of_countries(true);
    if (empty($mnethosts)) {
        $mnethosts = $DB->get_records('mnet_host', null, 'id', 'id,wwwroot,name');
    }
    // Disable foreach of query countries
